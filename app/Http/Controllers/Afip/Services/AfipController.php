...
use App\Models\Afip\DdjjEmpresasModelo;
use App\Models\Afip\DeclaracionesJuradasModelo;
use App\Models\Afip\DetalleAfipHeaderModelo;
use App\Models\Afip\SubsidioSanoModelo;
use App\Models\Afip\SubsidioSuma70Modelo;
use App\Models\Afip\SubsidioSumaModelo;
use App\Models\Afip\SubsidioSumarteModelo;
...
class AfipController extends Controller
{
    ...
    public function listAfip(Request $request)
    {
        ...
        if (!empty($request->cuil) && $cuilField) {
            $query = $query->where($cuilField, 'LIKE', "$request->cuit%");
        }

        if (!empty($request->locatario) && $request->tipo != '1') {
        ...
        $query->orderByDesc('fecha_proceso');
        $result = $query->get();

        return response()->json($result, 200);
    }

     public function getPorAfiliado(Request $request)
    {
        $declaraciones = DeclaracionesJuradasModelo::with('PadronAfil', 'Empresa')
            ->where('cuil', $request->cuil)
            ->whereNotIn('cuit', ['33648104389', '30537364919'])
            ->orderByDesc('fecproc')
            ->get();

        $transferencias = TransferenciasModelo::with('PadronAfil', 'Empresa')
            ->where('cuitapo', $request->cuil)
            ->whereNotIn('cuitcont', ['33648104389', '30537364919'])
            ->orderByDesc('fecproc')
            ->get();
        $data = [];
        foreach ($declaraciones as $key) {
            $data[] = [
                "cuit" => $key->cuit,
                "empresa" => $key->empresa->razon_social ?? 'Empresa No registrada',
                "cuil" => $key->cuil,
                "nombre" => !is_null($key->PadronAfil) ? trim($key->PadronAfil->nombre) . ' ' . trim($key->PadronAfil->apellidos) : 'Afiliado Sin Identificar',
                "remu" => $key->remimpo,
                "fechPre" => $key->fecpresent,
                "fechPro" => $key->fecproc,
                "tipo" => '1'
            ];
        }
                    ...
                    if ($files[0] != '') {
                        if ($request->tipoArchivo == '1') {
                            if (substr($files[0], 0, -2) != 'TFOS') {
                                ...
                                $remcont = (substr($files[31], 0, -2) . '.' . substr($files[31], -2));
                                $apobsoc = (substr($files[26], 0, -2) . '.' . substr($files[26], -2));
                                $conos = (substr($files[27], 0, -2) . '.' . substr($files[27], -2));
                                $remtot = (substr($files[28], 0, -2) . '.' . substr($files[28], -2));
